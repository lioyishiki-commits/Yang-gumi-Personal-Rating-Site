...
需要给别人查看时，使用项目内的 `启动只读分享.bat` 或主站“数据管理”中的“开始公网分享”。分享程序会单独创建带临时令牌的 HTTPS 只读链接，不会把主站端口直接暴露到局域网或互联网。该方式不要求修改 IP、网卡、NAT、系统代理、路由器端口或防火墙，也不依赖 VPS、域名和专用账号。
...
1. 主人先启动主站，再双击 `启动只读分享.bat`，或在“数据管理”中开始公网分享。
2. 等待显示临时 HTTPS 链接，并保持分享窗口运行。
3. 访客在任意可联网电脑的 Chrome、Edge 或 Firefox 中打开该链接即可，不需要安装 Python 或下载整个项目。
4. 如果访客使用 Windows，也可以接收主人成功启动后自动更新过的 `启动只读分享.bat`。这个单独文件会自动寻找主人当前有效的临时链接。
5. 主人停止分享后，原链接立即失效；下次启动可能生成新链接。

分享站仅显示“首页”“条目库”“排行榜”“评分对比”“标签筛选”“评分设置”。“评分设置”只读展示评分维度和比重，并提供不含私人备注、本机路径和维护数据的评分明细 XLSX。
