...
# See tools/plain_tunnel.LICENSE. The CLI wrapper below keeps this project
# independent from the larger Plain framework package.
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    print(f"PLAIN_TUNNEL_URL={self.tunnel_http_url}", flush=True)
        ...
        async with httpx.AsyncClient(
            ...
            try:
                async with client.stream(
                    ...
                    self.logger.info(
                        f"{request_metadata['method']} {request_metadata['url']} {response_status}"
                    )
                ...
                self.logger.info(
                    f"{request_metadata['method']} {request_metadata['url']} 502"
                )
                await self._handle_buffered_response(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Account-free Plain Tunnel client for Yang-gumi"
    )
    ...
    try:
        ...
        print(
            f"PLAIN_TUNNEL_ERROR={type(exc).__name__}: {exc}",
            file=sys.stderr,
            flush=True,
        )
        return 1
