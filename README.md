...
当前功能版本为 `1.2.8`。更新器只采用 GitHub `VERSION` 文件中的正式版本号，不再根据提交数量自行猜测版本。破坏性或不兼容更新提升主版本，新功能或较大范围更新提升次版本，修复与小幅调整提升补丁版本。手动检查更新会在确认更新后安全重启正在运行的本目录网站，使新代码立即生效；未运行的网站不会被意外启动。GitHub 下载包中的 Windows 批处理直接保存为 CMD 安全编码与 CRLF 换行，可由 `cmd.exe` 稳定执行。
