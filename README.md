...
- JSON、CSV、XLSX 评分明细、SQLite 备份与恢复
...
当前功能版本为 `1.2.10`。更新器只采用 GitHub `VERSION` 文件中的正式版本号，不再根据提交数量自行猜测版本。破坏性或不兼容更新提升主版本，新功能或较大范围更新提升次版本，修复与小幅调整提升补丁版本。手动检查更新会在确认更新后安全重启正在运行的本目录网站，使新代码立即生效；未运行的网站不会被意外启动。GitHub 下载包中的 Windows 批处理直接保存为 CMD 安全编码与 CRLF 换行，可由 `cmd.exe` 稳定执行。更新器会在下载筛选后和实际写入前分别校验目标路径；只要发现数据库、评分/收藏记录、今日美图设置、图片索引、本地海报或背景进入更新计划，就会停止更新。

## 公网交互式只读分享

1. 在主人电脑的项目目录中双击 **`启动只读分享.bat`**，或在主站“数据管理”中点击“开始公网分享”。
2. 等待页面显示临时 HTTPS 链接；分享进程保持运行时，主人在主站保存的修改会自动反映到分享站。
3. 访客只需要浏览器。分享站保留“首页”“条目库”“排行榜”“评分对比”“标签筛选”“评分设置”六个页面，不能新增、编辑或删除数据。
4. 分享站“评分设置”页面可以下载不含私人备注和本机路径的全部评分明细 XLSX；其他分享页面不放置下载按钮。
5. 如需让另一台 Windows 电脑长期跟随临时链接变化，可在主人成功启动分享后，将自动更新过的 **`启动只读分享.bat`** 单独复制给访客；访客不需要 Python 或项目文件。

此流程不要求主人和访客处于同一网络，也不要求修改 IP、网卡、NAT、系统代理、路由器端口或防火墙规则；不需要购买 VPS、域名或专用账号。链接包含临时访问令牌，只发送给需要查看的人；主人停止分享后，原链接立即失效。
